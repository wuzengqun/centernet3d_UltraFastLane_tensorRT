...
import glob
import matplotlib
# matplotlib.use('Agg')   # 非交互式后端，不依赖窗口
...
center_input_h = 384   #3d模型输入尺寸
center_input_w = 1280
lane_input_h = 288     #车道线检测模型输入尺寸
lane_input_w = 800
...
VIDEO_SAVE = cv2.VideoWriter('./3D_lane.mp4', fourcc, 25.0, (1679, 376))  
...
def draw_bev_cv(results3d, origin_image, save_video=True):
    # 创建空白 BEV 图
    ...
    bev_img_resized = cv2.resize(bev_img, (int(bev_img.shape[1] * h / bev_img.shape[0]), h))
    # 调试用
    # h = origin_image.shape[0]
    # combined_width = origin_image.shape[1] + bev_img_resized.shape[1]
    # print('combined_height, combined_width', h, combined_width)  # 用这个值去初始化out，写入视频

    combined = np.hstack((origin_image, bev_img_resized))
    if save_video:
        VIDEO_SAVE.write(combined)
    cv2.imshow("3D-bev", combined)
    cv2.waitKey(1)
...
def load_images_or_video(path):
    ...
    if os.path.isdir(path):
        ...
        for p in image_paths:
            img = cv2.imread(p)
            if img is not None:
                images_in_memory.append((os.path.basename(p), img))
        t_load_end = time.time()
        print(f"Loaded {len(images_in_memory)} images from folder in {t_load_end - t_load_start:.3f}s")
        ...
        while True:
            ...
            if not ret:
                break
            images_in_memory.append((f"frame_{frame_id:06d}.png", frame))
            frame_id += 1
        cap.release()
        t_load_end = time.time()
        print(f"Loaded {len(images_in_memory)} frames from video in {t_load_end - t_load_start:.3f}s")

    else:
        raise ValueError(f"Path is neither a folder nor a supported video file: {path}")
...
def main():
    ...
    # image_folder_or_video = './project_video.mp4'  # mp4文件
    image_folder_or_video = '/home/b541-2/deeplearning_project/wzq/detect-3d/qd_3dt_modified-main/dataset/kitti_raw/2011_09_29/2011_09_29_driver_0071_sync/2011_10_03_drive_0047_sync/2011_10_03/2011_10_03_drive_0047_sync/image_02/data/'   # 可以是文件夹或mp4文件
    images_in_memory = load_images_or_video(image_folder_or_video)

    for filename, origin_image in images_in_memory:
        ...
        # ==== CenterNet3D  推理 ====
        np.copyto(center_inputs[0].host, center_image.ravel())
        ...
        processed_outputs = [out.reshape(-1) for out in center_trt_outputs]
        boxes2d, results3d = center_postprocess(processed_outputs, calibs)
        ...
        draw_bev_cv(results, origin_image, False)
